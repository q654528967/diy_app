		...
		<view v-for="(item, index) in culterList" :key="index" class="">
				...
				<!-- <uni-swiper-dot class="uni-swiper-dot-box" @clickItem=clickItem :item.children="item.children" :current="current" :mode="mode" :dots-styles="dotsStyles" field="content"> -->
					<swiper class="swiper-box indexSwiperHeight" :current="swiperDotIndex">
						<swiper-item v-for="(box, k) in item.children" :key="k">
							<view @click.stop="buyOrChange(box)" class="swiper-item" :class="'swiper-item' + n">
			...
			<view v-if="item.rowType==2" class="">
				<view class="title">
					...
					<view class="titleContent">{{item.typeContent == null ? '' : item.typeContent}}</view>
				</view>
				<view class="gift">
					<view v-for="(box, n) in item.children" :key="n" class="giftBox">
						<image @click.stop="buyOrChange(box)" :src="box.proImgs" mode="widthFix" />
						<text @click.stop="changeList(item)" class="inChage" style="swiper">即刻兑换</text>
					</view>
				</view>
			</view>
			<!-- 限时兑换 -->
			<view v-if="item.rowType==3" class="exchange">
				<view style="position: relative;" class="changeTitle">
					...
					<image class="changeTitleImg" src="../../static/index/cult_xsdh.png" mode="widthFix" />
				</view>
				<uni-swiper-dot class="uni-swiper-dot-box" @clickItem=clickItem :item.children="item.children" :current="current" :mode="mode" :dots-styles="dotsStyles" field="content">
					<swiper class="swiper-box" :current="threeswiperDotIndex">
						<swiper-item v-for="(rowbox, index1) in item.children" :key="index1">
							<view class="swiper-item" :class="'swiper-item' + index1">
								<image @click.stop="buyOrChange(rowbox)" :src="rowbox.proImgs" style="width: 100%" />
								<view class="changeGoods">
									<view class="left">
										<view style="color: #151515;" class="changeName">{{rowbox.name == null ? '' : rowbox.name}}</view>
										<view class="price"> ￥{{rowbox.salePrice == null ? 0 : rowbox.salePrice}} <text style="text-decoration:line-through;margin: 0 15rpx;">￥{{rowbox.price == null ? 0 : rowbox.price}}</text> {{rowbox.pointsValue == null ? 0 : rowbox.pointsValue}}积分</view>
									</view>
									<view @click.stop="shopclick(rowbox)" class="right">
										<image src="../../static/index/shopIcon.png" mode="scaleToFill" />
									</view>
								</view>
							</view>
			...
			<view v-if="item.rowType==4" class="newGoods">
				<view style="position: relative;" class="newGoodsTitle">
					<text class="newTitle">{{item.name == null ? '' : item.name}}</text>
					<image class="changeTitleImg" src="../../static/index/cult_xpfb.png" mode="widthFix" />
				</view>
				<!-- 新品 -->
				<view class="goodsList">
					<view  v-for="(xinbox, index2) in item.children" :key="index2" :style="{backgroundImage:`url(${xinbox.proImgs})`,backgroundSize: 'cover'}" class="listBox" @click.stop="buyOrChange(xinbox)">
						<view class="lisName">{{xinbox.name == null ? '' : xinbox.name}}</view>
						<view class="btn">已售{{xinbox.saleNum == null ? 0 : xinbox.saleNum}}件</view>
						<view class="price">
							<view class="left">
								<view class="newPrice">￥{{xinbox.salePrice == null ? 0 : xinbox.salePrice}}</view>
								<view class="old1Price">{{xinbox.pointsValue == null ? 0 : xinbox.pointsValue}}积分</view>
							</view>
			...
			<view style="margin-top: 60rpx;" v-if="item.rowType==5" class="">
				<view style="position: relative;" class="newGoodsTitle">
					<text class="newTitle">{{item.name == null ? '' : item.name}}</text>
					<image class="changeTitleImg" src="../../static/index/jjyp.png" mode="widthFix" />
				</view>
				<TodayBuy @imageclick='changeList(item)' :proData="item.children" :jjimage='item.typeImg' @buyOrChange="buyOrChange" @shopclick='shopclick'></TodayBuy>
				<view @click="changeList(item)" class="more">更多</view>
			</view>
		</view>
